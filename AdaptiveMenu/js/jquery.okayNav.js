...
(function(factory) {
  ...
  function Plugin(element, options) {
    ...
    this.options.parent == ""
      ? (this.options.parent = self.navigation.parent())
      : "";
  ...
  $.extend(Plugin.prototype, {
    init: function() {
      ...
      if (self.options.align_right) {
        self.navigation
          ...
          .append(
            '<a href="#" class="' +
              self.options.toggle_icon_class +
              ' okay-invisible">' +
              self.options.toggle_icon_content +
              "</a>"
          );
      } else {
        self.navigation
      ...
      self.toggle_icon = self.navigation.children(
        "." + self.options.toggle_icon_class
      );
      ...
      self.nav_visible
        ...
        .filter(function() {
          return (this.nodeType =
            Node.TEXT_NODE && /\S/.test(this.nodeValue) === false);
        })
        .remove();
    ...
    _triggerMove: function(x, y) {
      ...
      if (angle < 20) {
        ...
        self.nav_invisible.css(
          "transform",
          "translateX(" + (self.options.align_right ? 1 : -1) * threshold + "%)"
        );
      }
    },
    ...
    recalcNav: function() {
      ...
      var wrapper_width = $(self.options.parent).outerWidth(true),
        ...
        collapse_width =
          self.nav_visible.outerWidth(true) + self.toggle_icon_width,
        expand_width =
          space_taken + self.last_visible_child_width + self.toggle_icon_width,
    ...
    //   console.log(self.nav_visible.children().length);
      self.options.itemDisplayed.call();
    },

    _expandAllItems: function() {
      var self = this;
      $("li", self.nav_invisible)
        .detach()
        .appendTo(self.nav_visible);
      self.options.itemDisplayed.call();
    },
